 ...
 class ODLine {
    ...
    getCoords() {
        ...
        // queryTask.execute(query).then(function (results) {
            for (let k = 0; k < results.features.length; k++) {
                let paths = results.features[k].geometry.paths[0];
                //将点转成成对坐标点
                let linePointsArray = [];
    ...
    addPathPoints(linePoints) {
        ...
        for (let k = 0; k < linePoints.length; k++) {
            ...
            this.opts.externalRenderers.toRenderCoordinates(this.view, [linePoints[k][0][0], linePoints[k][0][1], 10], 0, this.view.spatialReference, startPoint, 0, 1);
            this.opts.externalRenderers.toRenderCoordinates(this.view, [linePoints[k][1][0], linePoints[k][1][1], 10], 0, this.view.spatialReference, endPoint, 0, 1);
        ...
        for (let i = 0; i < linePoints.length; i++) {
            ...
            this.opts.externalRenderers.toRenderCoordinates(this.view, [point1[0], point1[1], 10], 0, this.view.spatialReference, xyz1, 0, 1);
            this.opts.externalRenderers.toRenderCoordinates(this.view, [point2[0], point2[1], 10], 0, this.view.spatialReference, xyz2, 0, 1);
